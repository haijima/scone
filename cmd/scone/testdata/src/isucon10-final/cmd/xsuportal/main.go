...
func (*AdminService) ListClarifications(e echo.Context) error {
	...
	if err != sql.ErrNoRows && err != nil {
		return fmt.Errorf("sql clarifications: %w", err)
	}
	...
	for _, clarification := range clarifications {
		...
		if err != nil {
			return fmt.Errorf("sql team(id=%v, clarification=%v): %w", clarification.TeamID, clarification.ID, err)
		}
...
func getCurrentContestant(e echo.Context, db sqlx.Queryer, lock bool) (*xsuportal.Contestant, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sql contestant: %w", err)
	}
...
func getCurrentTeam(e echo.Context, db sqlx.Queryer, lock bool) (*xsuportal.Team, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sql team: %w", err)
	}
	xc.Team = &team
	return xc.Team, nil
}
...
func getCurrentContestStatus(e echo.Context, db sqlx.Queryer) (*xsuportal.ContestStatus, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sql contest status: %w", err)
	}
