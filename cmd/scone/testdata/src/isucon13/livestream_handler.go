...
func searchLivestreamsHandler(c echo.Context) error {
	...
	if c.QueryParam("tag") != "" {
		...
		if err != nil {
			return echo.NewHTTPError(http.StatusInternalServerError, "failed to construct IN sql: "+err.Error())
		}
		...
		if c.QueryParam("limit") != "" {
			...
			if err != nil {
				return echo.NewHTTPError(http.StatusBadRequest, "limit sql parameter must be integer")
			}
