...
var (
	...
	ErrInvalidMasterVersion     = fmt.Errorf("invalid master version")
	ErrInvalidItemType          = fmt.Errorf("invalid item type")
	ErrInvalidToken             = fmt.Errorf("invalid token")
	ErrGetRequestTime           = fmt.Errorf("failed to get request time")
	ErrExpiredSession           = fmt.Errorf("session expired")
	ErrUserNotFound             = fmt.Errorf("not found user")
	ErrUserDeviceNotFound       = fmt.Errorf("not found user device")
	ErrItemNotFound             = fmt.Errorf("not found item")
	ErrLoginBonusRewardNotFound = fmt.Errorf("not found login bonus reward")
	ErrNoFormFile               = fmt.Errorf("no such file")
	ErrUnauthorized             = fmt.Errorf("unauthorized user")
	ErrForbidden                = fmt.Errorf("forbidden")
	ErrGeneratePassword         = fmt.Errorf("failed to password hash") //nolint:deadcode
)
...
func (h *Handler) receivePresent(c echo.Context) error {
	...
	query2, params, err := sqlx.In(query, req.PresentIDs)
	if err != nil {
		return errorResponse(c, http.StatusBadRequest, err)
	}
	obtainPresent := []*UserPresent{}
	if err = h.DB.Select(&obtainPresent, query2, params...); err != nil {
		return errorResponse(c, http.StatusBadRequest, err)
	}
