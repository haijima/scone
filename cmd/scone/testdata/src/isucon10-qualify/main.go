...
func searchChairs(c echo.Context) error {
	...
	//scone:sql SELECT COUNT(*) FROM chair WHERE price >= ? AND height >= ? AND width >= ? AND depth >= ? AND kind = ? AND color = ? AND features LIKE CONCAT('%', ?, '%') AND stock > 0
	...
	//scone:sql SELECT * FROM chair WHERE price >= ? AND height >= ? AND width >= ? AND depth >= ? AND kind = ? AND color = ? AND features LIKE CONCAT('%', ?, '%') AND stock > 0 ORDER BY popularity DESC, id ASC LIMIT ? OFFSET ?
...
func searchEstates(c echo.Context) error {
	...
	//scone:sql SELECT COUNT(*) FROM estate WHERE door_height >= ? AND door_width >= ? AND rent >= ? AND features like concat('%', ?, '%')
	err = db.Get(&res.Count, countQuery+searchCondition, params...)
	if err != nil {
	...
	estates := []Estate{}
	params = append(params, perPage, page*perPage)
	//scone:sql SELECT * FROM estate WHERE door_height >= ? AND door_width >= ? AND rent >= ? AND features like concat('%', ?, '%') ORDER BY popularity DESC, id ASC LIMIT ? OFFSET ?
